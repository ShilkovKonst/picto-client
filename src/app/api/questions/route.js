...
export async function POST(req) {
  ...
  // send request to server with body, create new question
  const accessToken = req?.cookies?.get("accessToken");
  const formData = await req.formData();
  try {
    const response = await fetch(`${process.env.SERVER_BASE_URL}/questions`, {
      method: "POST",
      body: formData,
      headers: {
...
async function handleGetAll(accessToken, pageNo, listSize) {
  const response = await fetch(
    `${process.env.SERVER_BASE_URL}/questions?page=${pageNo}&size=${listSize}`,
    {
      headers: {
        Authorization: `Bearer ${accessToken.value}`,
      },
...
async function handleGetAllAsList(accessToken) {
  const response = await fetch(
    `${process.env.SERVER_BASE_URL}/questions?asList=true`,
    {
      headers: {
        Authorization: `Bearer ${accessToken.value}`,
      },
